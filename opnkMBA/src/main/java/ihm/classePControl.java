...
public class classePControl {


	private TableView<Seance> SeanceTable;
	// pour laquelle on n'affiche que quatre informations (id, nom, prénom et motDePasse)
	@FXML
	private TableColumn<Seance, String> IDCoursColonne;
	@FXML
	private TableColumn<Seance, String> IDSalleColonne;
	@FXML
	private TableColumn<Seance, String> DateDebColonne;
	@FXML
	private TableColumn<Seance, String>DateFinColonne;
	...
	public void setMainApp(AuthGUI mainApp) {
		...
		//SeanceTable.setItems(this.mainApp.getSeanceDATA());
		afficheSéanceSelection();  
	}

	/**
	 * Mets � jour la partie gauche de la fen�tre avec les d�tails de l'Séance actif 
	 */
	private void afficheSéanceSelection() {
		Seance SéanceCourant = SeanceTable.getItems().get(ligneActive);
		//identifiant.setText(SéanceCourant.getID()+"");

		//motDePasse.setText(SéanceCourant.getmotDePaase());
	...
	private void handleMouseTableViewAction(MouseEvent event) {
		...
		this.setLigneActive(SeanceTable.getSelectionModel().getSelectedIndex());
		this.afficheSéanceSelection();
	}
